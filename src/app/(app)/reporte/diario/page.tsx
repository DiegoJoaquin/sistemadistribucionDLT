...
export default async function PaginaReporte(props: PageProps<"/reporte/diario">) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <SelectorFecha fecha={fecha} ruta="/reporte/diario" />
      </div>

      <Nota>
        El reporte que se envía ahora es el{" "}
        <Link href="/reporte" className="underline underline-offset-2">
          semanal
        </Link>
        . Este queda disponible para revisar un día puntual, y es el único que
        todavía tiene las preguntas de texto libre.
      </Nota>
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(0,26rem)_minmax(0,1fr)]">
        ...
        <VistaPreviaReporte
          html={html}
          texto={texto}
          nombreArchivo={`reporte-kpis-${fecha}`}
          titulo="Vista previa del correo diario"
        />
      </div>
    </div>
  );
