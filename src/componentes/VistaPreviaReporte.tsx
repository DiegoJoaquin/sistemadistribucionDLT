 ...
 *
 * El envío queda para después; por ahora se copia o se descarga.
 */
export function VistaPreviaReporte({
  ...
  const descargar = () => {
    ...
    a.href = url;
    a.download = `reporte-kpis-${fecha}.html`;
  ...
  return (
    <section className="tarjeta overflow-hidden">
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[var(--color-filete)] px-4 py-3">
        <div>
          <h2 className="text-sm font-semibold">Vista previa del correo</h2>
      ...
      {aviso && (
        <p role="status" className="border-b border-[var(--color-filete)] bg-[var(--color-realce)] px-4 py-2 text-sm text-[var(--color-tinta-suave)]">
          {aviso}
        </p>
      )}

      <iframe
        title="Vista previa del reporte diario"
        srcDoc={html}
